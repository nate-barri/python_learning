...
from utils.db import (
    ...
    get_players_by_team, get_top_assist_providers
)
...
def leaderboard_row(rank, player, team, value, max_val, color):
    c1, c2, c3, c4, c5 = st.columns([1, 3, 3, 1, 4])
    c1.markdown(f"<p style='margin:8px 0; font-size:18px;'>{medal(rank)}</p>", unsafe_allow_html=True)
    c2.markdown(f"<p style='margin:8px 0; font-weight:600;'>{player}</p>", unsafe_allow_html=True)
    c3.markdown(f"<p style='margin:8px 0; color:#6B7280;'>{team}</p>", unsafe_allow_html=True)
    c4.markdown(f"<p style='margin:8px 0; font-weight:700; color:{color}; font-size:16px;'>{value}</p>", unsafe_allow_html=True)
    c5.markdown(bar(value, max_val, color), unsafe_allow_html=True)
    st.markdown("<hr style='margin:2px 0; border:none; border-top:1px solid #F9FAFB;'>", unsafe_allow_html=True)


# ══════════════════════════════════════════════════════════════════════════
# TAB 1 — Top Scorers, Assists, Passes to Goals, Cards
# ══════════════════════════════════════════════════════════════════════════
with tab1:
    sub1, sub2, sub3, sub4 = st.tabs([
        "Top Scorers", "Top Assists", "Passes to Goals", "Disciplinary"
    ])
    ...
    with sub1:
        ...
        df = get_top_scorers()
        if df.empty:
            st.info("No goal data yet.")
        else:
            for i, row in df.iterrows():
                leaderboard_row(i+1, row["player_name"], row["team"],
                                row["goals"], df["goals"].max(), "#1F4E79")

    with sub2:
        st.markdown("### Top Assists")
        df = get_top_assists()
        if df.empty:
            st.info("No assist data yet.")
        else:
            for i, row in df.iterrows():
                leaderboard_row(i+1, row["player_name"], row["team"],
                                row["assists"], df["assists"].max(), "#2E75B6")

    with sub3:
        st.markdown("### Passes Leading to Goals")
        st.caption("Players who provided the assist pass on a goal — direct passes that led to a goal being scored.")
        df = get_top_assist_providers()
        if df.empty:
            st.info("No assist data yet. Run fetch_all_events() first.")
        else:
            for i, row in df.iterrows():
                leaderboard_row(i+1, row["player_name"], row["team"],
                                row["goal_assists"], df["goal_assists"].max(), "#059669")

    with sub4:
        st.markdown("### Disciplinary")
        df = get_top_cards()
        if df.empty:
            st.info("No card data yet.")
        else:
...
with tab2:
    ...
    if df.empty:
        ...
        category = st.radio(
            "View", ["Attacking", "Passing", "Defending", "Discipline", "Per 90"],
            horizontal=True
        )
...
with tab4:
    ...
    col_team, col_player = st.columns([2, 3])

    with col_team:
        selected_team = st.selectbox("Filter by Team", ["All Teams"] + team_names, key="dive_team")

    # Load player list based on team selection
    if selected_team == "All Teams":
        player_names = get_all_player_names()
    else:
        player_names = get_players_by_team(selected_team)

    if not player_names:
        st.info("No player stats yet. Run fetch_player_stats() first.")
        st.stop()

    with col_player:
        selected_player = st.selectbox("Select Player", player_names, key="dive_player")
    ...
    if df.empty:
        ...
        avg_rating  = round(df[df["rating"] > 0]["rating"].mean(), 2) if (df["rating"] > 0).any() else 0
        ...
        key_p90     = round(key_passes / total_min * 90, 2) if total_min > 0 else 0
        ...
        with col_pass:
            ...
            st.markdown(f"Key Passes per 90: **{key_p90}**")
